...
export default function DreamDetail() {
  ...
  function handleCopyPrompt() {
    if (analysis?.video_prompt) {
      ...
      toast.success('咒语已复制到剪贴板');
    }
  }

  async function handleDelete() {
    if (!confirm('确定让这段梦境消散？')) return;
    await api.dreams.delete(id);
    navigate('/');
  }

  if (loading) return <div className="text-center py-12 text-slate-500">回忆浮现中...</div>;
  if (!dream) return <div className="text-center py-12 text-slate-500">这段梦境已消散</div>;
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 text-sm text-slate-500">
            ...
            {dream.sleep_quality > 0 && <span>安眠: {'★'.repeat(dream.sleep_quality)}</span>}
          </div>
        </div>
      ...
      {analysis ? (
        <div className="space-y-6">
          ...
          <div>
              <h3 className="text-sm font-medium text-slate-400 mb-2">情绪光谱</h3>
            <EmotionChart emotion={analysis.emotion} />
          </div>

          {analysis.themes?.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-slate-400 mb-2">梦的主题</h3>
          ...
          {analysis.symbols?.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-slate-400 mb-2">梦中意象</h3>
          ...
          {analysis.storyboard?.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-slate-400 mb-2">画面分镜</h3>
      ...
      {analysis && (
        <div className="space-y-4">
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <Video className="w-5 h-5 text-purple-400" /> 梦境影像
          </h2>
          ...
          {dream.video?.status === 'completed' ? (
            ...
            <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-amber-200 font-medium">视频密钥未配置</p>
                  <p className="text-sm text-amber-200/70 mt-1">前往设置配置视频服务，或复制下方咒语到即梦平台施展</p>
                </div>
              </div>
              ...
              {analysis.video_prompt && (
                <div className="p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                  <h4 className="flex items-center gap-1 text-xs text-slate-500 mb-1">
                    <FileText className="w-3 h-3" /> 视频咒语（可复制到其他平台使用）
                  </h4>
              ...
              <div className="flex gap-3">
                ...
                >
                  <ExternalLink className="w-4 h-4" /> 前往即梦施展
                </a>
              </div>
            </div>
          ) : (
